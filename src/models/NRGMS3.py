...
class NRGMS3(OneTowerBaseModel):
    ...
    def infer(self, x):
        ...
        his_news_repr = his_news_repr.view(-1, self.title_length, 768)  #bert 出来
        ...
        cdd_news_reprs = self.attcdd(cdd_news_repr) #[20,5,128]
        al=his_corr *cdd_news_reprs.unsqueeze(dim=-2)  #orch.Size([2, 5, 50, 128])
        att = self.att(al)
        att_weights = self.softmax(att)#【2，5，50，1】
        user_repr1 = torch.matmul(his_corr.transpose(-1,-2), att_weights).squeeze(dim=-1) #torch.Size([2, 5, 128]

        user = self.user(torch.cat([user_repr1,user_repr], dim=-1)) #[2,5,128]

        sc = cdd_news_repr * user
        print(sc.size())
        logits = self.score(sc).squeeze(dim=-1)
