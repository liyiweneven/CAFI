...
class NRGMSNewsEncoder(BaseNewsEncoder):
    def __init__(self, manager):
    ...
    def forward(self, token_id, attn_mask=None, token_weight=None):
...
class CnnNewsEncoder(BaseNewsEncoder):
    def __init__(self, manager):
        ...
        self.news_query = nn.Parameter(torch.randn((1, 256), requires_grad=True))
        nn.init.xavier_normal_(self.news_query)
        self.newsProject = nn.Linear(256, 256)
        nn.init.xavier_normal_(self.newsProject.weight)
        self.Tanh = nn.Tanh()
        self.Relu = nn.ReLU()
...
class AttnUserEncoder(BaseUserEncoder):
    def __init__(self, manager):
        ...
        self.user_query = nn.Parameter(torch.randn((1, 256), requires_grad=True))
