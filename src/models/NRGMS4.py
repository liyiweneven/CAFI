...
class NRGMS4(OneTowerBaseModel):
    ...
    def infer(self, x):
        ...
        #对历史新闻做多头自注意力
        his_news_repr, h_query = self.multihead_self_attention(his_news_repr)  # [6,50,128] [6,50,128]

        # x的变换 input [6,5,128] -->[6,5,1,128]--->[6,5,50,128]
        cdd_trans = cdd_news_repr.unsqueeze(dim=2).repeat(1, 1, self.his_size, 1)
        print(cdd_trans.size())
        # -----v的内部信息流生成-----
        ...
        user_repr = self._scaled_dp_attention(self.query_news, finall_hisnews, finall_hisnews).view(self.batch_size,self.cdd_size,128)

        #按照相关加权
        #泛化版本

        cdd_news_reprs = self.attcdd(cdd_news_repr).unsqueeze(dim=-2) #[20,5,128]
        att_weights = self.softmax(torch.matmul(cdd_news_reprs, his_corr.transpose(-1, -2)))
        finall_hisnews2=(his_corr * att_weights.transpose(-1,-2)).view(-1, self.his_size, 128)  ##torch.Size([20, 5, 50, 128])
        user_repr2 = self._scaled_dp_attention(self.query_news1, finall_hisnews2, finall_hisnews2).view(self.batch_size,self.cdd_size, 128)

        user = self.user(torch.cat([user_repr, user_repr2], dim=-1)) #[2,5,128]
        logits = torch.matmul(cdd_news_repr.unsqueeze(dim=-2), user.unsqueeze(dim=-1)).squeeze(dim=-1)
