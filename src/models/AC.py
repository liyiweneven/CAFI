...
class AC(OneTowerBaseModel):
    def __init__(self, manager):
        ...
        self.affine5 = nn.Linear(in_features=256, out_features=128, bias=True)
        self.affine6 = nn.Linear(in_features=128, out_features=1, bias=False)

        self.lin3 = nn.Linear(in_features=1024, out_features=768, bias=False)
    ...
    def _cnn_resnet(self, C): #经过3个卷积核元素之间的间距不同的卷积 并且拼接在一起
        # C [30,20,768]
        C_fin = torch.zeros((*C.shape[:2], self.level,self.cnn_dim), device=C.device)#[30, 20, 3, 768]
        C1= C.transpose(-2, -1)
        C1 = self.CNN_d1(C1).transpose(-2,-1)
        Cnn1=self.Attention(C1)
        C_fin[:, :, 0, :] = self.ReLU(C1)
        C_fin[:, :, 0, :] = C1#[30, 20, 3, 128]


        C2 = C.transpose(-2, -1)
        C2 = self.CNN_d2(C2).transpose(-2,-1)
        Cnn2 = self.Attention(C2)
        C_fin[:, :, 1, :] = self.ReLU(C2)
        C_fin[:, :, 1, :] = C2

        C3 = C.transpose(-2, -1)
        C3 = self.CNN_d3(C3).transpose(-2,-1)
        Cnn3 = self.Attention(C3)
        C_fin[:, :, 2, :] = self.ReLU(C3)
        C_fin[:, :, 2, :] = C3

        TRC = torch.cat([Cnn1,Cnn2,Cnn3], dim=-1)#[30, 20,384]

        return C_fin,TRC#[30, 20, 3, 128]
    ...
    def infer(self, x):
        ...
        mul=cdd_trans*his_trans
        his_news = torch.cat([cdd_trans, cut, his_trans,mul], dim=-1)  # [6, 5, 50, 768][1024]
        # 做拼接后将其送入3个全连接层
        his_news=self.lin3(his_news)
        ...
        print(cnn_output.size(),his_news_output.size())
