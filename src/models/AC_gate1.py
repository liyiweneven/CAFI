...
class AC_gate1(OneTowerBaseModel):
    ...
    def infer(self, x):
        ...
        cdd_trans = self.lin3(cdd_news.unsqueeze(dim=2).repeat(1, 1, self.his_size, 1))  # [6,5,50,256]

        # -----v的内部信息流生成-----
        qu_intra = self.query_inte(h_query)  # torch.Size([6, 50, 128])
        mu_intra = self.mu_inte(his_news_repr)  # torch.Size([6, 50,128])
        v = (qu_intra + mu_intra).unsqueeze(dim=1)  # 【6，1,50，128】

        # -----g的外部信息流生成-----
        qu_gate = self.query_gate(h_query).unsqueeze(dim=1)  # torch.Size([6,1, 50, 128])
     ...
     ##-----------------------------------------------------------------------------------------------------------------------------
        his_corr=his_corr.view(-1,self.his_size,self.cnn_dim)
        out = torch.matmul(cnn_output, his_corr.transpose(-1, -2)).transpose(-1, -2)  # [30, 50, 5]
        user_repr = self._scaled_dp_attention(self.query_news, out, out)
        user_repr = self.Attention1(user_repr).unsqueeze(dim=1)  # [30,1,5]
