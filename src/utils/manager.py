...
class Manager():
    ...
    def __init__(self, config=None, command=None):
        """
        set hyper parametersabs

        Args:
            config: some extra configuration
        """
        parser = argparse.ArgumentParser()
        # 实验数据集的规模【demo,small,large】
        parser.add_argument("-s", "--scale", dest="scale", help="data scale", type=str, choices=["demo", "small", "large", "whole"], default="demo")
        # 训练模型多少个epoch
        parser.add_argument("-e", "--epoch", dest="epochs", help="epochs to train the model", type=int, default=10)
        # 模型的训练方式 【train dev test】
        ...
        #parser.add_argument("--device", dest="device",help="device to run on", choices=["0", "1", "cpu"], default="cpu")
        # 模型多少batch训练一次
        ...
        parser.add_argument("--metrics", dest="metrics", help="metrics for evaluating the model",type=str, default=["auc", "mean_mrr", "ndcg@5", "ndcg@10"])
        ...
        parser.add_argument("-lr", "--learning-rate", dest="learning_rate", help="learning rate", type=float, default=2e-5)
        ...
        parser.add_argument("-dr", "--data-root", dest="data_root", default="/News-Recommendation/Data")
        parser.add_argument("-cr", "--cache-root", dest="cache_root", default="data/cache")

        parser.add_argument("-tl", "--title-length", dest="title_length", type=int, default=20)
        ...
        parser.add_argument("-ef", "--enable-fields", dest="enable_fields", help="text fields to model",  type=str, choices=["title", "abs"], default="title")
        ...
        parser.add_argument("-cd", "--cnn-dim", dest="cnn_dim", type=int, default=128)
    ...
    def save(self, model, step, best=False):
        ...
        if isinstance(model, torch.nn.parallel.DistributedDataParallel):
            model = model.module
        if best:
            save_path = f"/News-Recommendation/src/data/ckpts/{model.name}/{self.scale}/best.model"
        else:
            save_path = f"/News-Recommendation/src/data/ckpts/{model.name}/{self.scale}/{step}.model"
    ...
    def train(self, model, loaders):
        ...
        if self.rank == 0:
            ...
            os.makedirs(f"/News-Recommendation/src/data/ckpts/{model.module.name if isinstance(model, torch.nn.parallel.DistributedDataParallel) else model.name}/{self.scale}", exist_ok=True)
